...
デフォルトではクリッカブルにしない設計になっています。何かクリック等のアクションを受け入れるのであれば、 a 要素、 button 要素、 [role="button"] 属性などを採用します。
...
<Canvas>
  <Story name="ARIA button role">{html`
    <div class="jumpu-tag" tabindex="0" role="button" aria-pressed="false">
      デフォルト
    </div>
  `}</Story>
</Canvas>
...
これはボタンの中にボタンがあります、混乱の元となり推奨できません。
