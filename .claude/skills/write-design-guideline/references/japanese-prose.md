...
`packages/docs` のページを書く・推敲するときは、以下の規範に従う。

出典は [k16shikano による Gist](https://gist.github.com/k16shikano/fd287c3133457c4fd8f5601d34aa817d)（Unlicense）。書籍原稿を対象にした規範を、docs のページ向けに改めたもの。

## 整形

- 一文ごとに改行する。段落の区切りは空行で示す。
- コード、差分、ログ、設定ファイルの断片はコードブロックで示す。
- 用語の由来や制約の背景など、本筋から一段外れる補足は、本文に並べず `> [!NOTE]` に降ろす。
...
- 実際に描画して見せる例は `:::raw` で囲み、直後に同じマークアップをコードブロックで置く。描画とコードが食い違ったまま残らないよう、片方だけを直さない。
- 注記は `> [!NOTE]`、守らないと壊れるものは `> [!IMPORTANT]` を使う。実験的な機能や書き換えを伴う操作の警告は `::: warning` を使う。
- 一文ごとに改行する規則は prettier と衝突しない。`proseWrap` は既定値の `preserve` なので改行はそのまま残る。
...
- 「他のページで扱う」のような前方参照は、論証が一段落した位置（段落末・節末）に置く。論証の途中に挟んで流れを切らない。
...
- ページ・節をまたいで、同じ概念の扱いを一致させる。ある節で「人間が決める」と分類したものを、別の節で「チームで合意する」と書かない。分類・定義・用語の地位は全体で揃える。
...
- 論証の中で読者を「あなた」と呼ばず、役割名（「開発者」「読者」）で書く。二人称の呼びかけは、場面への導入（「〜としよう」）やページの結びなど、限られた要所にとどめる。
- 対象を指す語は具体的に選ぶ。「AI」「ツール」のような広い語でぼかさない。
- ページや節で術語（トークン、尺度、マークアップ規約、パターンなど）を導入したら、以後はその語で通す。「文脈」「ツール」「AI」のような曖昧語に後退しない（定式化する前の導入語として「文脈」などを使うのはよい）。
...
Jumpu UI の術語（尺度、マークアップ規約、ユーティリティフレンドリーなど）を議論に使うのはよい。空虚な装飾として使うのが問題である。
